...
def analyze_mental_health_risk_reliable(text):
    """
    RELIABLE RULE-BASED SYSTEM WITH SENTIMENT ANALYSIS
    Much more predictable and accurate than pure semantic similarity
    """
    ...
    negative_life_phrases = [
        'don\'t want to live', 'do not want to live', 'dont want to live',
        'not want to live', 'no longer want to live',
        'don\'t want to be alive', 'do not want to be alive'
    ]
    
    has_negative_life = any(phrase in text_lower for phrase in negative_life_phrases)
    
    if has_negative_life:
        ...
        return {
            ...
            'detected_factors': ['🔴 Expresses not wanting to live'],
            'sentiment': sentiment,
    ...
    positive_indicators = [
        ...
        'trying to live', 'love life', 'enjoy life', 'life is good', 
        'life is beautiful', 'life is worth', 'grateful', 'blessed',
        'happy', 'excited', 'wonderful', 'amazing', 'great day',
        'looking forward', 'hopeful', 'optimistic', 'feeling better'
    ]
    
    has_positive = any(phrase in text_lower for phrase in positive_indicators)
    
    # If positive content OR very positive sentiment → LOW RISK
    if has_positive or sentiment > 0.4:
        return {
            'risk_level': 'Low',
            'risk_score': 5,
            'confidence': 0.92,
    ...
    mental_health_keywords = [
        'die', 'death', 'kill', 'suicide', 'end', 'live', 'life',
        'sad', 'depressed', 'anxious', 'hopeless', 'worthless', 'alone',
        'empty', 'pain', 'hurt', 'suffer', 'struggle', 'burden',
        'hate', 'scared', 'afraid', 'worry', 'stress', 'lonely'
    ]
    
    has_mental_health_context = any(keyword in text_lower for keyword in mental_health_keywords)
    
    # If NO mental health context AND neutral/positive sentiment → UNRELATED
    if not has_mental_health_context and sentiment >= -0.2:
        return {
            'risk_level': 'Low',
            'risk_score': 5,
            'confidence': 0.95,
            ...
            'detected_factors': ['🟢 No suicide intent detected in this message',
                               '✅ Content unrelated to mental health crisis'],
            'sentiment': sentiment,
    ...
    for phrase, score in high_risk_phrases.items():
        if phrase in text_lower:
            risk_score += score
            detected_factors.append(f'🔴 High-risk phrase: "{phrase}"')
            break  # Only count one
    
    # MEDIUM-HIGH RISK INDICATORS (+25 to +35 each)
    medium_high_phrases = {
        'hate this life': 32,
        'hate my life': 32,
        'skeptical whether to live': 35,
        'skeptical to live': 35,
        'better off without me': 35,
        'burden to everyone': 30,
        'worthless': 28,
        'no hope': 30,
        'hopeless': 28,
        'can\'t go on': 30,
    ...
    for phrase, score in medium_high_phrases.items():
        if phrase in text_lower:
            risk_score += score
            detected_factors.append(f'🟡 Concerning phrase: "{phrase}"')
            break
    
    # ISOLATION / LONELINESS (+15 to +22)
    isolation_phrases = {
        'nobody cares': 22,
        'no one cares': 22,
        'all alone': 18,
    ...
    for phrase, score in isolation_phrases.items():
        if phrase in text_lower:
            risk_score += score
            detected_factors.append(f'🟡 Isolation: "{phrase}"')
            break
    
    # SENTIMENT ADJUSTMENT
    if sentiment < -0.4:
        adjustment = abs(sentiment) * 25
        risk_score += adjustment
        detected_factors.append(f'🟡 Very negative sentiment: {sentiment:.2f}')
    elif sentiment > 0.3:
        adjustment = sentiment * 20
        risk_score -= adjustment
    ...
    protective_phrases = {
        ...
        'getting therapy': -30,
        'but i have': -20,
        'talking to someone': -25
    }
    
    for phrase, score in protective_phrases.items():
        if phrase in text_lower:
            risk_score += score
            detected_factors.append(f'🟢 Protective factor: "{phrase}"')
    ...
    if not detected_factors:
        detected_factors.append('🟢 No significant risk indicators detected')
...
if page == "Home":
    ...
    with col1:
        ...
        <div style='background: linear-gradient(135deg, #667eea, #764ba2); padding: 30px; border-radius: 15px; color: white; text-align: center;'>
            <h3>🧠 Rule-Based + AI</h3>
            <p>Reliable phrase detection with sentiment analysis</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if st.button("🎯 ANALYZE RISK", use_container_width=True, type="primary"):
        if len(text_input.strip()) < 5:
            ...
            with st.spinner("🤖 AI analyzing with transformers..."):
                ...
                result = analyze_mental_health_risk_reliable(text_input)
                progress.empty()
